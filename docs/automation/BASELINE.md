...
Published candidate `c76bc50` was then rejected by GitHub Actions run
`29844155406`. Public metadata proves the canonical step exited 1 and retained
its artifact; exact logs remain authentication-gated. Independent fresh-clone
reproduction with workflow uv 0.11.29 and managed CPython 3.12.13 deterministically
passes seven gates and rejects only `rel001-benchmark` with
`published_report_drift`. A control with the same uv and CPython 3.12.3 matches
the old report, falsifying uv and Go cleanup as causes. The non-runtime
difference is exactly Python version, the Python component digest, aggregate
structural digest, and lifecycle digest. The correction is a repository and CI
pin to CPython 3.12.13 plus an unedited report generated by the three-run
benchmark. RED/GREEN and differential evidence is under
