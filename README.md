...
> **Current status — pre-release prototype.** The canonical statement of
> implemented, experimental, and planned support is
...
installed `ucf ratchet establish`, `evaluate`, and `advance`. A separately
...
Revision-bound evidence, release benchmarks, and release acceptance remain
...
| Baseline and ratchet | Experimental language-neutral ratchet 1.0.0 on the same checked fixture: exact allowances, semantic/observed touch selection, regression reports, protected improvements, and immutable successors |
...
| Generation | Experimental exact request/result 1.0.0 plus one external Python/pytest action-function backend, deterministic receipt-backed generated-only publication, clean pytest 9.0.2 execution, and protected user implementation; the legacy source-YAML skeleton remains separate |
...
| Stable target | Broader ecosystem/platform support beyond the exact checked procedures, three-fixture benchmarks, and release acceptance remain planned; formal `verified` claims are unavailable |
...
reviewed onboarding bundle + exact assessment + accepted ratchet baseline
          |
          v
stable subject fingerprints -> pure evaluation report -> explicit successor
                         unchanged / touched / resolved / regression
...
[docs/ONBOARDING.md](docs/ONBOARDING.md). The separate baseline comparison,
atomic transaction, and trust-anchor limits are documented in
...
| [docs/RATCHET.md](docs/RATCHET.md) | Exact ratchet 1.0.0 documents, touch/classification rules, immutable successor transaction, safety, and measured limits |
