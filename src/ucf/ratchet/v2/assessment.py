...
from ucf.onboarding import CaptureContext, OnboardingBundle
from ucf.ratchet.v2.errors import RatchetErrorCode, RatchetValidationError
from ucf.ratchet.v2.identity import derive_assessment_id, derive_violation_id
...
def build_ratchet_assessment(
    ...
    *,
    ...
    unknown_partial = set(partial_rule_ids) - {rule.id for rule in policy.rules}
    if unknown_partial:
    ...
    coverage = tuple(
        RuleCoverage(
            ...
            rule=_rule_ref(rule),
            status="partial" if rule.id in partial_rule_ids else "complete",
    ...
    provisional = RatchetAssessment(
        ...
        source=onboarding_bundle_ref(bundle),
        producer=producer,
        procedure_uri=procedure_uri,
        capture_context=capture_context,
        behavior=BehaviorAssessmentLedger(
            kind="ratchet_behavior_assessment",
            subjects=subjects,
            coverage=coverage,
            violations=tuple(
