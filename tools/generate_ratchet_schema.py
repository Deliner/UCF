"""Generate deterministic UCF ratchet profile 1.0.0 schemas."""
...
DEFAULT_OUTPUT_DIRECTORY = (
    REPOSITORY_ROOT / "src" / "ucf" / "schemas" / "ratchet" / "v1"
)
...
_DOCUMENTS = (
    (
        ...
        RATCHET_POLICY_SCHEMA_URI,
        "ratchet_policy",
        "UCF ratchet policy profile 1.0.0",
    ),
    (
        ASSESSMENT_SCHEMA_OUTPUT,
        RatchetAssessment,
        RATCHET_ASSESSMENT_SCHEMA_URI,
        "ratchet_assessment",
        "UCF ratchet assessment profile 1.0.0",
    ),
    (
        ...
        RATCHET_BASELINE_SCHEMA_URI,
        "ratchet_baseline",
        "UCF ratchet baseline profile 1.0.0",
    ),
    (
        EVALUATION_REPORT_SCHEMA_OUTPUT,
        RatchetEvaluationReport,
        ...
        "UCF ratchet evaluation report profile 1.0.0",
    ),
)
    ...
    schema.update(
        {
            "$comment": (
                ...
                "1.0.0 models; do not edit by hand. Content identities, "
                "canonical order, resolved references, exact source "
                "projections, predecessor lineage, and recomputed summaries "
                "remain runtime semantics."
            ),
            "$id": schema_uri,
            ...
            "x-ucf-runtime-semantic-checks": [
                ...
                (
                    "content-derived policy, subject, violation, assessment, "
                    "baseline, and report identities"
                ),
                (
                    "exact policy, onboarding bundle, assessment, baseline, "
                    "predecessor, and report references"
                ),
                (
                    "canonical unique rule, subject, violation, "
                    "classification, allowance, and protection order"
                ),
                "resolved non-provenance behavior subject projection",
                "resolved rename-neutral observed evidence projection",
                (
                    "complete subject and rule coverage before baseline "
                    "creation or advance"
                ),
                (
                    "recomputed unchanged, touched, regression, resolution, "
                    "and unknown classifications"
                ),
                (
                    "immutable predecessor-bound tightening with retained "
                    "protected resolutions"
                ),
                "explicit review delta without implicit baseline weakening",
...
def _parse_args(arguments: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate or check the published ratchet schemas."
    )
    parser.add_argument(
        "--output-directory",
        type=Path,
